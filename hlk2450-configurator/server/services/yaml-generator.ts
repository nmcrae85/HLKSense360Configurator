import { SensorConfiguration, Zone } from '@shared/schema';

export class YAMLGeneratorService {
  public generateESPHomeConfig(config: SensorConfiguration): string {
...
`;
  ...
  private generateTargetSensors(): string {
    let yaml = '';
...
`;
  ...
  private generateZoneNumbers(zones: Zone[]): string {
    let yaml = '';
...
`;
  ...
  private getZoneBounds(zone: Zone): { x1: number, y1: number, x2: number, y2: number } {
  ...
  public generateAdvancedConfig(config: SensorConfiguration): string {
    // Generate configuration with TillFleisch external component for polygon zones
...
`;
  ...
  private generatePolygonZones(zones: Zone[]): string {
  ...
  private generateAdvancedTargetSensors(): string {
    let yaml = '';
...
export const yamlGenerator = new YAMLGeneratorService();
